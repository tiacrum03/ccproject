# Bug Report 
Description: Project will consist of a tutorial on how to create a bug report using Jira.
Bug reports are a great way for QA testers and Developers to communicate and collaborate to fix errors and stay in scope of business requirements of an application etc.
<h1> Environment and Technologies used </h1>

- Windows 7
- Remote laptop
- Jira


<h2>Demonstration</h2>
- After setting up a Jira acccount and creating a project name, you are going to select 'Create'
<img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTC56xsJO-gJmiuv8GHu52d88T29Hdqn-aWeA&usqp=CAU" height="80%" width="80%" alt="Demonstration"/>


 <h2>Steps</h2>

- You will need to 
- select a 'Project' name
- select 'Bug' under 'Issue type'
- submit a brief 'Description' of bug under 'Summary'
- upload a video or image of the bug (optional) under 'Attachment'
- submit a 'Description' of bug (optional)
- select an 'Assignee'
- select 'Priority' rating
- select 'Create' at bottom of form when complete

<img src="https://miro.medium.com/max/1400/1*DlkxaovGTJDOAnq_EpSaKw.png" height="80%" width="80%" alt="Steps"/>

 
 
-  Once all neccessary details are filled out, go back to the home page and you will see the bug report you created &#128516;



           


  
